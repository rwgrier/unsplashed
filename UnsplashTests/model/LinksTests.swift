...
class LinksTests: XCTestCase {
    ...
    func testLinksFromJson() {
        guard let json = try? JSONSerialization.jsonObject(with: singleLink.data(using: String.Encoding.utf8)!, options: JSONSerialization.ReadingOptions(rawValue: 0)) as? [NSObject: AnyObject] else { fatalError() }
